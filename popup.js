...
"    filter: invert(1);"+
"}"+
"[role='main'], .pagehead, .repohead-details-container, .header, .site-footer-container, .js-repo-pjax-container," +
...
"}"+
".mini-repo-list-item .repo-icon, .repo-and-owner, .css-truncate-target," +
    ".css-truncate.css-truncate-target, a.js-navigation-open, h2,"+
    "[itemprop='name'] a, p, .message, [rel='author'], .repo.js-repo, .pl-c1"+
    "not:('.pl-en, .pl-en+.pl-c1, .pl-s+.pl-c1, .pl-smi+.pl-c1+.pl-c1'){"+
"color: #bbbbbb !important;"+
"}"+
".pl-en, .pl-en+.pl-c1, .pl-s+.pl-c1, .pl-smi+.pl-c1+.pl-c1{"+
    "color: #ffc66d"+
"}"+
".pl-s, .pl-pds{"+
    "color: #6a8759;"+
"}"+
".pl-c1+.pl-smi, .pl-smi+.pl-c1{"+
    "color: #9876aa;"+
"}"+
".pl-k{"+
    "color: #cc7832;"+
...
".blob-wrapper.data.type-javascript {"+
    "background-color: #2b2b2b !important;"+
"}"+
".blob-code-inner, .pl-en+.pl-c1+.pl-smi, .pl-c1+.pl-k, .pl-c1, a, .user-mention, .contributors-toggle{"+
    "color: #a9b7c6;"+
"}"+
".form-control{"+
...
var spans = document.getElementsByClassName('pl-c1');
for(var i = 0; i<spans.length;i++){

    if(spans[i].textContent == 'null' || spans[i].textContent == 'this'){
        spans[i].style.color = '#cc7832';
    }

    if(typeof parseInt(spans[i].textContent) == 'number'){
        spans[i].style.color = '#a9b7c6';
    }

    if(spans[i].previousSibling.textContent == "."){
        spans[i].style.color = "#9876aa";
    }

    if(spans[i].nextSibling.textContent.indexOf("(") == 0){
        spans[i].style.color = "#ffc66d";
    }
}
...
for(var n = 0; n<plSmis.length;n++){
    if(plSmis[n].previousSibling && plSmis[n].previousSibling.textContent == "."){
        plSmis[n].style.color = "#9876aa";
    }
}

var operators = document.getElementsByClassName('pl-k');
for(var x = 0; x<operators.length;x++){
    if(
        operators[x].textContent == '=' ||
        operators[x].textContent == '==' ||
        operators[x].textContent == '===' ||
        operators[x].textContent == '!=' ||
        operators[x].textContent == '!==' ||
        ...
        operators[x].style.color = '#bbbbbb';
    }
}
